...
- 候选展示不带头像，确认后显示所选老婆的 QQ 头像。
- 支持「重新挑选」换一批候选（次数可配置，默认 2 次，0 表示不限）与「放弃」（退还当日抽取名额）。
- 「挑选老婆」计入每日抽取上限（daily_limit），防止绕过。
- 候选抽取排除自己、机器人、排除列表及不在群成员；活跃池不足时自动降级候选人数。
- 选择超时（30 秒）自动取消；非法编号提示重新回复且保留请求。
- 新增配置项：`pick_candidate_count`（候选人数）、`pick_retry_limit`（重新挑选次数上限）。
- 文案中的「她」统一为「Ta」，避免性别歧义。
