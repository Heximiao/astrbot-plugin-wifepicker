...
新增 Telegram 平台适配：缓存群友昵称和数字用户 ID，支持回复消息或已知用户名指定求婚、强娶对象，支持头像缓存、成员状态检查和话题群。

Telegram 复用现有玩法、次数和冷却规则。修复挑选老婆在当天仍有候选批次时，放弃或超时后被冷却提前拦截的问题。

## 2026.9.4更新

新增 Discord 平台兼容：支持 Discord 成员昵称、头像、成员有效性过滤和机器人账号过滤。

新增英文命令别名；Discord 成员接口不可用时会自动回退到近期活跃用户池。

统一 OneBot 与 Discord 的成员资料读取入口，关系图、排行榜、抽取、挑选、强娶和求婚均可使用对应平台的成员资料。
